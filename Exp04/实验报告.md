...
graph TD
    ...
    B --> C[AuthServiceImpl.login 验证用户信息]
    C --> D[UserMapper.findByUsername 查询用户]
    D --> E{用户是否存在}
    E -->|否| F[抛出异常 用户不存在]
    E -->|是| G[BCryptPasswordEncoder.matches 验证密码]
    G --> H{密码是否正确}
    H -->|否| I[抛出异常 密码错误]
    H -->|是| J{用户状态是否正常}
    J -->|否| K[抛出异常 账号已被禁用]
    J -->|是| L[获取用户角色信息]
    L --> M[JwtUtil.generateToken 生成JWT Token]
    M --> N[返回LoginResponse包含Token等信息]
...
```mermaid
graph TD
    A[学生发起选课请求] --> B[StudentController.enrollCourse]
    B --> C[StudentServiceImpl.enrollCourse 执行选课逻辑]
    C --> D[TeachingClassMapper.findByIdWithDetails 查询教学班]
    D --> E{教学班是否存在}
    E -->|否| F[抛出异常 教学班不存在]
    E -->|是| G[检查教学班状态是否允许选课]
    G --> H{教学班是否为未开课状态}
    H -->|否| I[抛出异常 教学班当前不允许选课]
    H -->|是| J[EnrollmentMapper.findByStudentAndClass 检查是否已选]
    J --> K{是否已选过该课}
    K -->|是| L[抛出异常 您已经选过这门课了]
    K -->|否| M[检查教学班是否已满]
    M --> N{教学班是否已满}
    N -->|是| O[抛出异常 教学班已满 无法选课]
    N -->|否| P[创建Enrollment选课记录]
    P --> Q[EnrollmentMapper.insert 插入选课记录]
    Q --> R[TeachingClassMapper.incrementCurrentStudentsIfNotFull 更新教学班人数]
    R --> S{并发安全更新是否成功}
    S -->|否| T[抛出异常 教学班已满 无法选课]
    S -->|是| U[返回选课成功信息]
```
...
```mermaid
graph TD
    A[用户发起AI咨询请求] --> B[StudentController或TeacherController接收请求]
    B --> C[AIService.studentConsult 或 AIService.teacherConsult]
    C --> D[ScoreMapper.findByStudentId 或 ScoreService.getClassStatistics 查询数据]
    D --> E[构建成绩上下文信息作为提示词]
    E --> F[chatLanguageModel.generate 调用AI模型生成回答]
    F --> G{是否使用流式响应}
    G -->|是| H[AIService.studentConsultStreamingSync 或 teacherConsultStreamingSync]
    G -->|否| I[返回AI生成的完整回答]
    H --> J[StreamingResponseHandler.onNext 逐字返回AI响应]
    J --> K[前端实时显示流式AI回答]
    I --> L[前端显示AI回答]
```
...
```mermaid
graph TD
    A[教师录入成绩] --> B[TeacherController.inputScore]
    B --> C[TeacherServiceImpl.inputScore]
    C --> D[调用ScoreService.inputScore]
    D --> E[TeachingClassMapper.findByIdWithDetails 验证权限]
    E --> F{教师权限验证}
    F -->|否| G[抛出异常 无权限操作此教学班]
    F -->|是| H[EnrollmentMapper.findByStudentAndClass 查找选课记录]
    H --> I{选课记录是否存在}
    I -->|否| J[抛出异常 选课记录不存在]
    I -->|是| K[创建或更新Score对象]
    K --> L{是否所有成绩分项都有值}
    L -->|否| M[仅保存输入的成绩分项]
    L -->|是| N[按权重计算总分和绩点]
    N --> O[ScoreMapper.insert 或 update 保存到数据库]
    M --> P[返回部分成绩录入成功信息]
    O --> Q[返回完整成绩录入成功信息]
```
...
```mermaid
graph TD
    A[管理员填写学生信息] --> B[AdminController.createStudent]
    B --> C[AdminServiceImpl.createStudent]
    C --> D[StudentMapper.findByStudentNo 检查学号是否已存在]
    D --> E{学号是否已存在}
    E -->|是| F[抛出异常 学号已存在]
    E -->|否| G[UserMapper.findByUsername 检查用户名是否已存在]
    G --> H{用户名是否已存在}
    H -->|是| I[抛出异常 用户名已存在]
    H -->|否| J[创建User对象并加密密码]
    J --> K[UserMapper.insert 插入用户记录]
    K --> L[创建Student对象]
    L --> M[StudentMapper.insert 插入学生档案记录]
    M --> N[返回创建的学生信息]
    N --> O[返回创建成功信息]
```
...
```mermaid
graph TD
    A[管理员填写教学班信息] --> B[AdminController.createTeachingClass]
    B --> C[AdminServiceImpl.createTeachingClass]
    C --> D[CourseMapper.findById 验证课程是否存在]
    D --> E{课程是否存在}
    E -->|否| F[抛出异常 课程不存在]
    E -->|是| G[TeacherMapper.findById 验证教师是否存在]
    G --> H{教师是否存在}
    H -->|否| I[抛出异常 教师不存在]
    H -->|是| J[TeachingClassMapper.selectOne 检查教学班编号重复]
    J --> K{该学期教学班编号是否已存在}
    K -->|是| L[抛出异常 该学期教学班编号已存在]
    K -->|否| M[创建TeachingClass对象]
    M --> N[TeachingClassMapper.insert 插入教学班记录]
    N --> O[返回创建的教学班信息]
    O --> P[返回创建成功信息]
```
